...
function KitchenCard({
  ...
  const handleAdvance = async () => {
    ...
    setUpdating(true)
    try {
      await updatePanelEstado(pedido.id, panel, nextEstado)
  ...
  const panelItems = pedido.items?.filter((i) => i.producto?.panel === panel) ?? []
...
export function KitchenDisplay() {
  ...
  useEffect(() => {
    ...
    const active = pedidos.filter(p =>
      ...
      p.items?.some(i => i.producto?.panel === panel)
    ).length
  ...
  }, [pedidos, panel, playNotification])
